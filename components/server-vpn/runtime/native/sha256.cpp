...
std::uint32_t ReadBigEndian(const std::uint8_t* input) {
...
std::string Sha256Hex(std::string_view input) {
  ...
  for (std::size_t offset = 0; offset < padded.size(); offset += 64) {
    ...
    for (std::size_t index = 0; index < 16; ++index) {
      words[index] = ReadBigEndian(padded.data() + offset + index * 4);
    }
    for (std::size_t index = 16; index < words.size(); ++index) {
      const std::uint32_t left = std::rotr(words[index - 15], 7) ^
          std::rotr(words[index - 15], 18) ^ (words[index - 15] >> 3U);
      const std::uint32_t right = std::rotr(words[index - 2], 17) ^
          std::rotr(words[index - 2], 19) ^ (words[index - 2] >> 10U);
      words[index] = words[index - 16] + left + words[index - 7] + right;
    }
    auto [a, b, c, d, e, f, g, h] = state;
    for (std::size_t index = 0; index < words.size(); ++index) {
      const std::uint32_t upper = std::rotr(e, 6) ^ std::rotr(e, 11) ^ std::rotr(e, 25);
      const std::uint32_t choice = (e & f) ^ (~e & g);
      const std::uint32_t first = h + upper + choice + kRoundConstants[index] + words[index];
      const std::uint32_t lower = std::rotr(a, 2) ^ std::rotr(a, 13) ^ std::rotr(a, 22);
